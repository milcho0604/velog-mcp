...
이 세 도구만 브라우저를 쓰고 나머지 18개는 없어도 동작한다.
...
테스트 234건. `safety.test.ts` 가 보안 불변식(A1~A11)을, `render.test.ts` 가
그림 쪽 불변식(R1~R23)을 고정한다. 깨지면 우회하지 말고 왜 깨졌는지부터 볼 것.

여기 있는 방어는 전부 **일부러 망가뜨려** 확인했다. 변이 20종, 각각이 테스트를
정확히 1건씩 실패시켜야 한다. **방어를 지웠는데도 통과하는 테스트는 테스트가 아니다.**
