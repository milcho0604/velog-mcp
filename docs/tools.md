...
- **선 양끝은 면 가운데에 붙고 렌더러가 옮기지 않는다.** 곧은 선은 두 노드의 `y`
  (세로로 이으면 `x` 중심)를 맞춰 얻는다. 안 맞으면 완만할 때(1:4 이하) 가운데끼리 대각,
  가파르면 가운데서 한 번 꺾는다. 예전엔 양끝을 평균 자리로 끌어와 곧게 폈는데 둘 다
  중심에서 빗나갔다.
- **한 면에서 같은 방향으로 여러 선이 나가면(들어오면) 가운데 한 점을 같이 쓰고** 한
  자리에서 갈라진다. 나가는 선과 들어오는 선이 섞인 면만 화살촉이 안 겹치게 조금 벌린다.
...
| `cross` | 선이 노드를 관통하거나 노드 뒤로 숨음 · 선이 그룹 이름표를 가림 |
| `overlap` | 서로 다른 선이 같은 자리에 겹침 |
| `collide` | 노드끼리 겹침 · 배지가 아이콘 침범 |
| `label` | 라벨이 카드·다른 라벨·그룹 이름표 위에 얹힘 · 라벨이 그룹 테두리에 걸침 |
