...
```
받는 것    PNG · JPEG · GIF · WebP — 시작 시그니처 **와 끝맺음**이 모두 맞는 것
안 받는 것  그 외 전부 (SVG 포함) · 10MB 초과 · 디렉터리
           머리만 베껴 붙인 파일 · 잘린 파일 · IEND 뒤에 데이터를 덧붙인 PNG
```

머리 8바이트만 보면 "PNG 시그니처 + 아무 텍스트"가 통과한다. 그건 이미지가 아니라
**이미지처럼 시작하는 파일**이다. 끝맺음(PNG=IEND · JPEG=FFD9 · GIF=0x3B ·
WebP=RIFF 길이)까지 확인한다.
