...
- 작성: 2026-07-30
- 상태: 초안 (v0.1 기준, 갱신하지 않음)

> **정정 (0.4.2).** 이 문서는 v0.1 시점의 기획이다. G1 의 '발행 코드를 두지 않는다'와
> 이후의 초안 전용 서술은 [ADR 0004](decisions/0004-capability-model.md) 로 대체됐다.
> 지금은 초안과 비공개 발행이 기본이고, 공개 발행만 `VELOG_ALLOW_PUBLIC` 게이트를 탄다.
> 당시의 문제 인식을 남기기 위해 원문은 고치지 않는다.
