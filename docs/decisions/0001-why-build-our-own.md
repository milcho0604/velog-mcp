...
- 날짜: 2026-07-30
- 상태: 채택 (단, '발행 코드가 없다'는 서술은 [ADR 0004](0004-capability-model.md) 로 대체됨)

> **정정 (0.4.2).** 아래에서 '발행 mutation 을 호출하는 코드가 레포에 없다'고 적은
> 부분은 더 이상 사실이 아니다. 위험의 경계가 '발행'이 아니라 '공개'에 있다는 걸
> 실측으로 알고 ADR 0004 에서 권한을 사용자에게 돌려줬다. 지금은 초안과 비공개
> 발행이 기본이고 공개만 게이트를 탄다. **직접 만들기로 한 판단 자체는 유효하다.**
