...
계수 대상이 `is_private: false` 뿐이다. **비공개 글은 이 파괴적 조치를 유발할 수
없다.** 그리고 비공개 글은 RSS·검색·구독 메일 어디에도 안 나간다 — 지우면 정말
지워진다.
...
초안을 `is_private: true` 로 바꾸자 **그 위험이 원천 소멸했다.** 초안은 어차피
본인만 보므로 공개 여부에 의미가 없어 잃는 것도 없다. rate limit 은 이제 공개
발행 경로에만 남는다.

방어를 하나 더 얹는 것보다 **애초에 위험 구간에 안 들어가는 값**을 고르는 게
나은 예다.
