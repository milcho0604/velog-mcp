...
> **정정 (0.4.2, 2026-08-11).** 아래 결론은 더 이상 유효하지 않다. 게이트가 막아야
> 하는 것은 '안 보이던 글을 내보내는 것'이지 '이미 공개된 글을 내리는 것'이 아니다.
> 후자는 발행이 아니라 되돌리기 어려운 파괴적 변경이다. 지금은 게이트가 꺼져 있으면
> **공개 범위를 바꿀 수단 자체가 없고** 기존 값을 잇는다. 판단의 기록으로 남긴다.
