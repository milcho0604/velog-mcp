...
- 날짜: 2026-07-31
- 상태: 채택
- 숫자 정정: 2026-08-24 (아래 배너)

> ⚠️ **아래 본문의 도구 개수는 2026-07-31 당시 값이다.** 0.7.0 에서 시퀀스 다이어그램
> 도구 `velog_render_sequence` 를 넣어, 지금은 도구가 **22개**이고 크롬이 필요한 그림
> 도구는 `velog_render_diagram`, `velog_render_sequence`, `velog_render_cover` **셋**이다.
> 본문의 "둘"과 "21개"는 그때 무엇을 확인했는지의 기록이라 고쳐 쓰지 않는다
> (ADR 0001, 0002, 0004 와 같은 방식이다). 지금 값은 [`docs/tools.md`](../tools.md) 가 기준이다.
>
> 정정보다 중요한 건 **이 문서가 예고한 장치가 실제로 걸렸다**는 것이다. 아래
> "3차에서 나온 것" 절은 개수 대신 이름을 적고 렌더 호출 지점을 함께 세기로 하면서
> 「세 번째 도구가 렌더를 쓰기 시작하면 목록이 낡았다고 실패한다」고 적어뒀다.
> 그 세 번째 도구가 실제로 생겼고, `CHROME_TOOLS` 에서 이름 하나를 빼보면 지금도
> P22 가 «렌더를 부르는 도구와 CHROME_TOOLS 가 다르다» 로 멈춰 세운다(변이로 확인).
> 안내가 조용히 낡을 자리를 그때 미리 막아둔 셈이다.
