...
>
> **② 기존 공개 글을 수정하거나 초안으로 되돌릴 수 있다.**
> `velog_update_post` 는 공개 범위를 건드리지 않는다 — 게이트가 꺼져 있으면
> 바꿀 수단 자체가 없어 공개 글은 공개로 남는다(0.4.2 에서 고쳤다).
> 다만 `velog_unpublish_post` 로 초안으로 되돌리는 것은 여전히 가능하다. **글이 사라지지는 않지만
...
호출 경로가 존재하지 않는 것은 다르다.

⚠️ 범위는 **초안 도구**다. 발행은 `tools/publish.ts` 가 따로 담당하고 공개 여부는
`VELOG_ALLOW_PUBLIC` 게이트를 탄다 — [ADR 0004](decisions/0004-capability-model.md).
위 분석의 출처는 [ADR 0002](decisions/0002-draft-only-write.md) 이고, 그 결론은
대체됐지만 '불린 하나라 상수로 박아야 한다'는 분석은 그대로 유효하다.
...
```bash
# 공개 발행이 게이트를 반드시 거치는지
grep -rn "publicPublish" src/
