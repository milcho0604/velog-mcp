...
못 나간다. 크롬을 `--host-resolver-rules=MAP * ~NOTFOUND` 로 띄워 **DNS 를 통째로
막는다.** 아이콘도 전부 내장 도형이라 받아올 것이 없다. 보안 완화 플래그
(`--disable-web-security`·`--allow-file-access-from-files`·`--no-sandbox`)는
쓰지 않고, 이 역시 테스트가 검사한다(R3).
