...
claude mcp add velog -e VELOG_REFRESH_TOKEN=your_refresh_token \
  -- npx -y @milcho0604/velog-mcp@0.8.2
```
...
```json
{
  "mcpServers": {
    "velog": {
      "command": "npx",
      "args": ["-y", "@milcho0604/velog-mcp@0.8.2"],
      "env": {
...
417 tests (as of 0.8.2). `src/__tests__/safety.test.ts` pins the security
invariants (A1–A11), `render.test.ts` pins the diagram ones (R1–R23) and the
sequence ones (S1–S10), and `plugin.test.ts` pins the packaging ones (P1–P28) —
