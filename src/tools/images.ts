	...
	server.registerTool(
		'velog_render_diagram',
		{
			...
			description:
				...
				'브라우저 실측으로 정해지고, 글자 삐져나옴/선 관통/선끼리 교차와 겹침/살짝 어긋난 줄은 자가감사가 잡는다.\n' +
				...
				'**배치를 먼저 설계한다.** 카드는 격자(행 간격 200 안팎)에 두고, 옆으로 나란하면 y 를 같게, ' +
				'어긋낼 거면 카드 높이 이상 어긋낸다. 한 면에는 나가는 선만 또는 들어오는 선만 둔다. ' +
				'감사를 피하려고 points·label_at 을 쓰지 않는다(끝이 허공에 뜨거나 라벨이 선에서 떨어지면 감사가 잡는다). 걸리면 카드 자리를 고친다.\n' +
				`아이콘: ${ICON_NAMES.join(' ')}\n톤: ${TONE_NAMES.join(' ')}`,
			inputSchema: {
